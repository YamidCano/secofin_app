<template>
  <q-card class="q-ma-lg q-pa-lg">
    <div class="text-h6">Listado de Pensiones</div>
    <div>
      <q-table flat bordered separator="cell" :rows="pensiones" :columns="columns" :filter="filter" :pagination="pagination"
        no-data-label="No se encontraron resultados" no-results-label="No se encontraron resultados en la busqueda"
        row-key="name">
        <template v-slot:top-left>
          <q-btn outline color="primary" icon="mdi-plus" label="Crear" :to="{ name: 'pensionesCreate' }" />
        </template>
            ...
            <q-td key="actions" :props="props">
              <div class="q-gutter-sm">
                <q-btn v-if="props.row.status == 1" round outline color="primary" icon="mdi-pencil" size="sm"
                  @click="EditPensiones(props.row.id)">
  ...
  <MyAlertDialog v-model="alertDialog" :confirm="deletePensiones" :id="itemId" />
...
const getPensiones = async () => {
  try {
    const { data } = await servicePensiones.getPensiones();
    pensiones.value = data.data
  } catch (error) {
    console.error("Error al obtener Pensiones:", error);
    showNotify('negative', 'Error al obtener la lista de Pensiones');
  }
}

const EditPensiones = async (id) => {
  StoreEditData.setEditPensiones(id)
  router.push({ name: 'pensionesUpdate' })
}

const deletePensiones = async (id) => {
  try {
    loading.value = true;
    const { data } = await servicePensiones.deletePensiones(id);
    await getPensiones();
...
const updateStatus = async () => {
  try {
    loading.value = true;
    const { data } = await servicePensiones.updateStatus(elementID.value);
    await getPensiones()
...
const handleStatus = async (Id, newStatus) => {
  ...
  DialogText1Block.value = `¿Estás seguro de ${newStatus === 2 ? 'Inactivar' : 'Activar'} la Pensiones?`
