<template>
  <q-card class="q-ma-lg">
    <q-card-section>
      <div class="text-h6">Actualizar Cesantias</div>
...
const showCesantias = async () => {
  try {
    loading.value = true
    const { data } = await serviceCesantias.showCesantias(StoreEditData.EditCesantias);
...
const update = async () => {
  ...
  try {
    ...
    const { data } = await serviceCesantias.updateCesantias(StoreEditData.EditCesantias, requestData);
    showNotify('positive', data.message)
    router.push({ name: "cesantiasView" });
  } catch (error) {
    showNotify('negative', error.response?.data?.errors?.nombre || 'Error al actualizar la Cesantias');
  } finally {
    loading.value = false;
  }
}
